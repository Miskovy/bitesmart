...
async function api(url, opts = {}) {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      ...(tok ? { Authorization: `bearerAuth ${tok}` } : {}),
      ...(opts.headers || {}),
    },
  });
...
function extractAuth(data) {
  return {
    ...
    user:  data.user  || data.data || data.account || {},
  };
}
...
function AuthPage({ setPage, setAuthed, setCurrentUser, darkMode, isSignup, setDarkMode }) {
  ...
  const Field = ({ field, placeholder, type="text", icon }) => (
    ...
    <div className="mesh" style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24,background:t.bg,position:"relative"}}>
      ...
      <div className={`asc ${t.gl}`} style={{width:"100%",maxWidth:420,borderRadius:22,padding:"40px 36px"}}>
        ...
        <div style={{display:"flex",background:darkMode?"rgba(255,255,255,.05)":"rgba(0,0,0,.05)",borderRadius:10,padding:4,marginBottom:24}}>
          {["signin","signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setApiErr(""); }} className="btn-o"
              style={{flex:1,padding:"8px",borderRadius:8,fontSize:14,fontWeight:500,border:"none",
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {mode==="signup" && <Field field="name"     placeholder="Full name"       icon={<User size={16}/>}/>}
          <Field field="email"    placeholder="Email address"   type="email"   icon={<Mail size={16}/>}/>
          <Field field="password" placeholder="Password"                       icon={<Lock size={16}/>}/>
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:4}}>
        {links.map(l => (
          <button key={l.id} onClick={() => setPage(l.id)} className={`slnk ${page===l.id?"act":""}`}
            style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",border:"none",background:"transparent",
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const raw  = Array.isArray(data) ? data : (data.meals||data.data||data.items||data.logs||[]);
      ...
      <div className="afu" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={() => { setSnapMode(p=>!p); setShowForm(false); setSnapImg(null); setSnapRes(null); }}
            className="btn-o"
            style={{padding:"10px 16px",borderRadius:10,fontSize:14,display:"flex",alignItems:"center",gap:8,
      ...
      {snapMode && (
        <div className={`asc ${t.gl}`} style={{borderRadius:16,padding:"24px",marginBottom:20}}>
          ...
          <div onClick={() => fileRef.current?.click()}
            style={{border:`2px dashed ${snapImg?"#10b981":t.bdr}`,borderRadius:14,cursor:"pointer",
          ...
          <div style={{display:"flex",background:darkMode?"rgba(255,255,255,.05)":"rgba(0,0,0,.05)",borderRadius:10,padding:4,marginBottom:28}}>
            {["metric","imperial"].map(u => (
              <button key={u} onClick={() => setUnit(u)} className="btn-o"
                style={{flex:1,padding:"8px",borderRadius:8,fontSize:13,border:"none",fontFamily:"'DM Sans',sans-serif",
...
function AIChat({ darkMode }) {
  ...
  const openSession = async (sid) => {
    ...
    try {
      ...
      const raw  = Array.isArray(data) ? data : (data.messages||data.history||data.data||[]);
  ...
  const send = async (text = input.trim()) => {
    ...
    try {
      ...
      /* Normalise backend response — adjust fields if your API differs */
      const replyText = data.reply || data.message || data.content || data.response || "…";
      const newSid    = data.sessionId || data.session_id || data.chatId || activeId;
      ...
      if (newSid && newSid !== activeId) {
        setActiveId(newSid);
        const newSess = data.session || { id:newSid, title:text.slice(0,30) };
            ...
            sessions.map(s => {
              ...
              return (
                <div key={sid} onClick={() => openSession(sid)}
                  style={{padding:"8px 10px",borderRadius:8,fontSize:12,color:isAct?t.acc:t.t2,cursor:"pointer",
            ...
            msgs.map((m,i) => (
              <div key={i} className="afi"
                ...
                <div className={m.role==="user"?"bubble-u":"bubble-a"}
                  style={{maxWidth:"72%",padding:"12px 16px",fontSize:14,lineHeight:1.75,
