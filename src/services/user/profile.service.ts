...
import { userModes } from "../../interfaces/interfaces";
...
export const getMyProfile = async (
  ...
  const currentStreak = await updateLoginStreak(
    userId,
    timezone,
    offsetMinutes,
  );
...
export const updatemyProfile = async (
  ...
  const { medicalConditions, dietaryPreferences, targets, ...userCoreData } =
    data;
  ...
  if (Object.keys(userCoreData).length > 0) {
    updatePromises.push(
      db.update(users).set(userCoreData).where(eq(users.id, userId)),
    );
  ...
  if (targets && Object.keys(targets).length > 0) {
    ...
    if (existing) {
      updatePromises.push(
        db.update(userTarget).set(targets).where(eq(userTarget.userId, userId)),
      );
    } else {
      updatePromises.push(
        db.insert(userTarget).values({
          userId,
